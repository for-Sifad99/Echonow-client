    ...
    return (
        <nav className={`sidebar h-[100vh] overflow-x-hidden fixed top-16 left-0 dark:bg-[var(--dark-bg)] bg-[var(--white)] transition-transform duration-300 ease-in-out w-[288px] z-20 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className='mx-auto font-oxygen flex md:hidden items-center bg-[var(--accent-white)] justify-between text-sm pl-4 pr-1 w-[250.19px] h-10 rounded-xl z-50 mt-6 sm:mt-8 -mb-5' >
                <input type="text" placeholder='Search Here...' className='ml-2 bg-[var(--accent-white)] border-none outline-none' />
                <FiSearch className="stroke-[var(--primary)] dark:stroke-[var(--dark)] bg-[var(--secondary)] dark:bg-[var(--white)] p-[12px] h-[34px] w-[34px] rounded-xl cursor-pointer" />
            </div>
        ...
        ${openStg ? 'block' : 'hidden'} text-[] font-semibold`}
                    >
                        {/* Theme */}
                        <input type="checkbox" onChange={toggleTheme} defaultChecked={theme === 'dark'} />
                        <p className='swap-off absolute top-0 left-8 pl-4 border-l text-[var(--dark)] dark:text-[var(--base-100)] opacity-70 border-gray-400 flex items-center gap-1 cursor-pointer'><FiMoon />Dark Mode</p>
                        <p className='swap-on absolute top-0 left-8 pl-4 text-[var(--dark)] dark:text-[var(--base-100)] border-l border-gray-400 flex items-center gap-1  cursor-pointer'><IoSunnyOutline />Light Mode</p>
                    </label>
                </li>
                <li className='md:hidden'>
                        ...
                        >
                            <div className="flex items-center py-1 w-full">
                                ...
                                <div className="flex flex-col items-start justify-start opacity-90 ml-2 mr-auto">
                                    <p className='font-bold font-oxygen leading-2 mt-1 text-[var(--white)]'>
                                        {user?.displayName}
                                    </p>
                                    <p className='text-xs font-jost text-[var(--accent-white)]'>
                                        {user?.email}
                                    </p>
                                </div>
