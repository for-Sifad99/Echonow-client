        ...
        <div className="relative z-10 px-2 sm:mx-3 py-3 xl:max-w-[1366px] 2xl:max-w-[1728px] xl:mx-auto">
            {trendingArticles.length > 0 ? (
                <Swiper
                    ...
                    spaceBetween={10}
                    slidesPerView={1}
                    loop={trendingArticles.length > 1}
                    autoplay={{ delay: 3000, disableOnInteraction: false }}
                    breakpoints={{
                        0: { slidesPerView: Math.min(1, trendingArticles.length) },
                        640: { slidesPerView: Math.min(2, trendingArticles.length) },
                        768: { slidesPerView: Math.min(3, trendingArticles.length) },
                        1024: { slidesPerView: Math.min(4, trendingArticles.length) },
                        1536: { slidesPerView: Math.min(5, trendingArticles.length) },
                    }}
                >
                    {trendingArticles.map((article, idx) => (
                        <SwiperSlide key={idx}>
                            <div onClick={() => handleNavigate(article, article._id)} className="group relative w-full h-full md:max-w-xs 2xl mx-auto overflow-hidden shadow-lg">
                                <img
                                    src={article.image}
                                    alt="banner"
